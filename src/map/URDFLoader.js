  ...
  parse(content, workingPath = this.workingPath) {
    ...
    function processUrdf(data) {
      ...
      if (data instanceof Document) {
        ...
        children = [urdf.documentElement];
        // console.log('xml:', urdf.documentElement.nodeName);
      }
    ...
    function processRobot(robot) {
      // console.log('robot:', typeof robot);
      const elementChildren = Array.from(robot.childNodes).filter(node => node.nodeType === 1);
      // const elementChildren = [...robot.childNodes].filter(node => node.nodeType === 1);  // 1 = ELEMENT_NODE
      // console.log('robot children:', elementChildren.length);
      const robotNodes = [...elementChildren];
      ...
      links.forEach(l => {
        ...
        const childElements = robot.getElementsByTagName('child');
        let isRoot = true;
        for (let i = 0; i < childElements.length; i++) {
          const child = childElements.item(i);
          if (child.getAttribute('link') === name) {
            isRoot = false;
            break;
          }
        }
    ...
    function processJoint(joint) {
      ...
      const children = Array.from(joint.childNodes).filter(node => node.nodeType === 1);
    ...
    function processLink(link, visualMap, colliderMap, target = null) {
      ...
      const elementChildren = Array.from(link.childNodes).filter(node => node.nodeType === 1);
    ...
    function processLinkElement(vn, materialMap = {}) {
      ...
      const children = Array.from(vn.childNodes).filter(node => node.nodeType === 1);
      let material = null;
      ...
      children.forEach(n => {
        ...
        const type = n.nodeName.toLowerCase();
        if (type === 'geometry') {
          const childred = Array.from(n.childNodes).filter(node => node.nodeType === 1);
          // const childred = [...n.children];
          const geoType = childred[0].nodeName.toLowerCase();
          if (geoType === 'mesh') {

            const filename = childred[0].getAttribute('filename');
            const filePath = resolvePath(filename);

            // file path is null if a package directory is not provided.
            if (filePath !== null) {

              const scaleAttr = childred[0].getAttribute('scale');
              if (scaleAttr) {
